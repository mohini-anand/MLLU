...
arguments = transformers.TrainingArguments(num_train_epochs = 3, per_device_train_batch_size = 8  )
'''learning rate'''
...
def hp_space_arg():
    return {'learning_rate' : uniform(1e-5,5e-5)}
"""
compute_objective?? local_dir ??
"""
def comp_obj_arg(metrics):
    return metrics['eval_loss']

trainer = transformers.Trainer(args= arguments, train_dataset=train_data, eval_dataset= val_data, tokenizer= tokenizer ,model_init=finetuning_utils.model_init, compute_metrics=finetuning_utils.compute_metrics)
trial = trainer.hyperparameter_search(backend = 'ray', local_dir = '/scratch/ma5266/checkpoints',  direction='minimize', hp_space= hp_space_arg, compute_objective= comp_obj_arg ,n_trials= 5, search_alg = BayesOptSearch())
print(trial)